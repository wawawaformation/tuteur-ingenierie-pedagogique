...
Cette photographie est une **référence de non-régression**, pas une validation officielle.
...
Interprétation : 1/3 ou 2/3 → instabilité, à consigner comme telle ; 3/3 → FAIL reproductible, qui **redéfinit la baseline** pour ce scénario au lieu d'être traité comme un défaut à corriger tout de suite.

Un FAIL reproductible découvert ici ne bloque pas le refactoring : il devient le comportement de référence à préserver, et un chantier distinct. Ne pas modifier le runtime pour le faire disparaître pendant le lot 0.
...
1. le commit exact de `en_cours/` sur lequel la baseline est établie, et `git diff --name-only -- en_cours/` vide ;
...
7. le statut : diagnostic ou campagne officielle, selon la session utilisée (D6) ;
...
Attendu : les trois conservent leur verdict de la baseline du lot 0.
...
Attendu : verdicts identiques à la **baseline comportementale du lot 0**, et non à la photographie du dry-run pré-refactorisation, qui n'a pas la même valeur probante.
...
**Contrôle de sortie du lot A :** tout scénario rejoué doit conserver son verdict. Un seul basculement → reverter l'étape responsable, ne pas empiler.
...
Attendu : **14/14 PASS**, C0 conforme.
...
**Contrôle de sortie du lot B :** NOY001-NOY013 inchangés et C0 conforme. NOY014 est explicitement hors critère.
...
**Contrôle de sortie du lot C :** NOY003, NOY007, NOY010 inchangés.
...
Chaque run exige : condition A, copie neuve du candidat, contexte neuf, exécutant ne connaissant pas l'oracle, aucun persona, stimulus exact, verbatim conservé. `NOY014_1` et `NOY014_2` jamais dans la même conversation.

Les runs officiels doivent se faire dans la session Linux dédiée `claude-test` prévue par `validation/CLAUDE.md`. Des runs joués dans des sous-contextes d'une session de développement valent comme **diagnostic**, pas comme campagne officielle.
...
| **R10** | Les runs de validation sont joués hors session dédiée et pris pour des runs officiels. | moyenne | moyenne | §8.2. Distinguer explicitement diagnostic et campagne dans tout rapport. |
...
2. **Batterie hors NOY014 : 14/14 PASS.** Les 14 scénarios sont NOY001 à NOY011 (11), NOY012_1, NOY012_2 et NOY013. Joués en condition A, contextes neufs, dans la session dédiée, sur le candidat refactoré.
...
| **D6** | Les runs de validation seront-ils joués dans la session dédiée `claude-test` ? | **Oui pour les critères de sortie.** Des runs de diagnostic en sous-contexte sont acceptables pendant les lots, à condition d'être étiquetés comme tels. | Distinction diagnostic / campagne officielle (R10). |
