...
**Batterie entière rejouée sur le candidat corrigé, le même jour : 15/15 PASS.** Le portage ayant modifié le candidat, les résultats antérieurs ne le couvraient plus (`V31-ACT01-1` et `V31-ACT02-4` avaient été joués *avant* la correction ; les 4 autres n'avaient jamais tourné sur le candidat réel). Les 7 scénarios ont donc été passés sur la fixture corrigée, mêmes conditions : Facettes 1/1, Quiz 3/3, Planche météo 3/3, Étude de cas 1/1, Simulation 1/1, Brique 3/3, Rétrospective 3/3 — **résultats identiques type par type à la campagne d'origine**, et unanimité sur les quatre scénarios à charge de preuve. La paire contrastive `ACT02-1`/`ACT02-2` départage bien sur un thème professionnel identique, et `ACT02-4` traite explicitement `typical_moments` comme un indice de sélection, pas une condition exclusive — la clause d'ACT02 que ce scénario protège.

**Reste dû, nommé explicitement** : la non-régression de la baseline V2.1 (`validation/v2.1/non_regression/`, 15 scénarios) n'a pas été jouée sur ce candidat, alors que le portage a touché trois gabarits du socle hérité. Exigence distincte de la batterie V3.1, posée par `promesse.md` §0 et `base_de_travail.md` §13 : V3.1.0 n'est pas validée au sens du gel de non-régression tant qu'elle n'est pas passée.

- `.claude/CLAUDE.md` et `en_cours/CLAUDE.md` : état des versions aligné (catalogue implémenté, 14 gabarits dans l'arborescence du runtime, batterie 15/15, non-régression V2.1 signalée comme non faite). Au passage, `.claude/CLAUDE.md` décrivait encore `validation/v3.1/non_regression/` comme « encore vide » alors qu'il contient 7 scénarios depuis le 2026-09-03 — corrigé.
