...
* **Élicitation en tutorat individuel, en dialogue plutôt que par prérequis** : en tutorat en direct avec un seul apprenant (`SKILL.md`, usage 1), l'agent dispose d'un canal que n'a pas la rédaction de documents : il peut interroger l'apprenant en temps réel plutôt que de supposer un niveau. Ce n'est pas qu'une bonne pratique andragogique : c'est ce qui rend le reste du skill applicable. Deux techniques concrètes, à préférer à une simple liste de prérequis déclarée :
  ...
  * **Exemple personnel demandé** : *"Donne-moi un cas où tu aurais eu besoin de ça."* Un exemple pertinent et correctement transposé est une preuve plus forte qu'une reconnaissance verbale, mais cet acte reste au niveau de la compréhension : il ne prouve pas à lui seul une mise en pratique autonome (`etat_des_paliers.md`).

  **Lorsque le dialogue individuel en temps réel n'est pas possible ou pertinent** — par exemple dans un collectif large ou un travail asynchrone sans échange individualisé — le quiz d'auto-positionnement et la checklist auto-cochée (`activites_type/quiz.md`) peuvent fournir une approximation diagnostique. Ne pas confondre cette limite du dispositif avec une limite du canal oral : une autre activité orale peut attester un palier supérieur si elle rend réellement observable la performance correspondante (`etat_des_paliers.md`).
